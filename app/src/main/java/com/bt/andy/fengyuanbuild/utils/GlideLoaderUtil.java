...
public class GlideLoaderUtil {
    ...
    public static void showImageView(Context context, String url, ImageView imageview) {
        Glide.with(context)
                .load(url)// 加载图片
                .placeholder(R.drawable.icon_nonet)//图片加载出来前，显示的图片
                .error(R.drawable.icon_nonet)// 设置错误图片
    ...
    public static void showLocalGifImage(Context context, int gifID, ImageView imgeview) {
        if (null != context) {
            Glide.with(context).load(gifID).asGif().into(imgeview);
        }
    }
}
